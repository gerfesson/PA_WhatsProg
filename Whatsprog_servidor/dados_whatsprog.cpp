...
void Servidor::new_case1(tcp_winsocket &t)
{
    ...
    {
        // Procura se ja existe um cliente conectado com o mesmo login
        ...
        else
        {
            ...
            else
            {
                ...
                cout << "tamanho do vetor de user: " << this->LU.size();
...
DWORD WINAPI servidor(LPVOID lpParameter)
{
    ...
    while (!fim)
    {
        ...
        if (!(fim = !c.accepting())) //
        {
            ...
            cout << "\n "<<c<<"\n";
            cout << "\n"<<legal<<"\n";
            for (i=serv.LU.begin(); i!=serv.LU.end(); i++)
            {
                if (i->getSocket().connected())
                {
                    legal++;
        ...
        if (!fim)
        {
            ...
            if (iResult!=0)
            {
                ...
                cout << "tamanho do vetor de user: " << serv.LU.size();
            ...
            {
                cout << "5";
                while ( i!=usr.end() && !(i->getSocket().connected()) )
                {
                    cout << "6";
                    i->getSocket().close();
                    i = usr.erase(i);
